...
class VarDecl(Node):
    def __init__(self, var_type, name, assignment_tokens):
        self.type = var_type          # "NUM", "STR", "LST", "ARR"
        self.name = name              # The variable string identifier
        self.tokens = assignment_tokens  # Everything after the variable name

class KeyWord(Node):
    def __init__(self, block_type, condition_str):
        self.type = block_type        # "WHILE", "IF", "ELSE", "REPEAT"
        self.condition = condition_str  # Raw condition expression string

class Identifier(Node):
    def __init__(self, name, arg_tokens):
        self.name = name              # Function name (e.g., 'print')
        self.args = arg_tokens        # List of raw arguments/tokens passed inside
def get_ast(parsed):
    tree = []
    for line in parsed:
        if not line.strip():
